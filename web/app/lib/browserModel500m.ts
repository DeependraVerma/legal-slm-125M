// Browser-only in-browser inference via transformers.js for the 500M SFT
// model. Identical pattern to browserModel.ts (the 125M build) but its own
// module-level singleton state, pointed at the 500M ONNX repo — kept as a
// separate file rather than a shared parameterized loader so the two models'
// state can never cross-contaminate if both pages are visited in one tab.
